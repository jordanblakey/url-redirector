...
describe('generateChangelog', () => {
    let mockExecSync: (cmd: string) => string;
    let mockFs: Pick<typeof fs, 'ensureDirSync' | 'writeFileSync'>;
    let mockGenAI: typeof GoogleGenerativeAI;
    let mockLoadSecrets: () => Promise<void>;
    ...
    beforeEach(() => {
        ...
        mockFs = {
            ...
            writeFileSync: (path: string, content: string) => fsOperations.push({ op: 'writeFileSync', path, content }),
        };
    ...
    test('should generate changelog using Gemini API', async () => {
        ...
        expect(fsOperations.some(op => op.op === 'writeFileSync' && op.content === 'AI Generated Changelog')).toBe(true);
    });
    ...
    test('should fallback if GEMINI_API_KEY is missing', async () => {
        ...
        expect(fsOperations.some(op => op.op === 'writeFileSync' && op.content.includes('Changes since v1.0.0'))).toBe(true);
    });
