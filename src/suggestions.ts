...
const rawProductiveSites: SiteData[] = [
  ...
  {
    url: "wikipedia.org/wiki/Wikipedia:Community_portal",
...
export const productiveSites = rawProductiveSites.filter(
...
export function getThematicPair(): { source: string; target: string } {
  ...
  const sourceSite =
    distractingSites[Math.floor(Math.random() * distractingSites.length)];

  // B. Find approved antidote categories
  const targetCategories =
    categoryMap[sourceSite.category as DistractionCategory];
  ...
  const candidates =
    matchingTargets.length > 0 ? matchingTargets : productiveSites;
