...
import { getAccessToken, uploadExtension, publishExtension, updateStoreListing, getStoreListing } from './cws-utils';
    ...
    if (dryRun) {
        ...
        if (changelog && descriptionTemplate && GEMINI_API_KEY) {
            ...
            if (currentListing) {
                ...
                await updateStoreListing(accessToken, EXTENSION_ID, newDescription, newPromotionalText, fetchFn);
...
function getVersionType(currentVersion: string, previousVersion: string | null): 'major' | 'minor' | 'patch' | 'initial' {
    if (!previousVersion) {
        return 'initial';
    }
